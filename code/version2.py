...
def deepnn(x):
  ...
  W_fc1 = weight_variable([int(d/8 * d/8 * 128), 1024])
  b_fc1 = bias_variable([1024])

  h_pool3_flat = tf.reshape(h_pool3, [-1, int(d/8*d/8*128)])
...
def getData(path):
  ...
  train = data[100:,:]
  test = data[0:100,:]
  train_means = np.mean(train[:,1:], axis=0)
...
def main(_):
  ...
  # Create the model
  x = tf.placeholder(tf.float32, [None, d*d])
  ...
  with tf.Session() as sess:
    ...
    for i in range(750):
      ...
      train_step.run(feed_dict={x: randy_feats, y_: randy_labels, keep_prob: 0.9})

    test_feats = data["test"][:,1:]
    test_labels = np.zeros((100,2))
    for j in range(100):
